...
final class VoiceSettings: ObservableObject {
    ...
    @Published var enabled: Bool {
        didSet { UserDefaults.standard.set(enabled, forKey: "doris.voice.enabled") }
    }
    ...
    private static let bindingsKey = "doris.voice.bindings.v1"

    private init() {
        let d = UserDefaults.standard
        self.enabled = (d.object(forKey: "doris.voice.enabled") as? Bool) ?? true
        ...
        if let data = d.data(forKey: Self.bindingsKey),
           ...
           !decoded.isEmpty {
            ...
            let oldKey = d.string(forKey: "doris.voice.triggerKey")
                .flatMap(TriggerKey.init(rawValue:)) ?? .rightControl
            let oldLang = d.string(forKey: "doris.voice.language")
                .flatMap(VoiceLanguage.init(rawValue:)) ?? .auto
            let oldProvider = d.string(forKey: "doris.voice.provider")
                .flatMap(Provider.init(rawValue:)) ?? .chatGPT
            let oldAutoSubmit = (d.object(forKey: "doris.voice.autoSubmit") as? Bool) ?? true
            self.bindings = [
                VoiceBinding(
                    triggerKey: oldKey,
