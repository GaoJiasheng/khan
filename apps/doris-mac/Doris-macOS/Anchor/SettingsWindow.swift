...
private struct AppearanceSettingsView: View {
    ...
    private var voiceSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text(L(
                "Hold a trigger key, speak, release. Doris transcribes locally with Apple Speech, then activates the chosen app and pastes (with optional auto-submit). First use prompts for Microphone, Speech Recognition, Input Monitoring, and Accessibility.",
                "按住触发键说话,松开后 Doris 用 Apple Speech 本地转写,自动前置目标 App 并粘贴(可选自动回车)。首次使用会逐步请求麦克风、语音识别、输入监听及辅助功能权限。"
            ))
            .font(.caption)
            .foregroundStyle(.secondary)
            .fixedSize(horizontal: false, vertical: true)
