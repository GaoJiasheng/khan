...
final class SpeechRecognizer {
    ...
    init(locale: Locale = .current) {
        ...
        KhanLog.voice.info("recognizer locale=\(pick.identifier, privacy: .public) requested=\(locale.identifier, privacy: .public) supportedCount=\(supported.count, privacy: .public)")
    }
    ...
    func start() throws {
        ...
        guard let recognizer, recognizer.isAvailable else {
            KhanLog.voice.error("recognizer not available (locale unsupported / offline model missing)")
        ...
        KhanLog.voice.info("input node sampleRate=\(format.sampleRate, privacy: .public) channels=\(format.channelCount, privacy: .public)")
        ...
        inputNode.installTap(onBus: 0, bufferSize: 1024, format: format) { [weak request] buf, _ in
            ...
            if bufferCount == 1 || bufferCount == 30 {
                KhanLog.voice.info("audio buffers tapped: \(bufferCount, privacy: .public)")
            }
        }

        engine.prepare()
        try engine.start()
        KhanLog.voice.info("audio engine running")

        task = recognizer.recognitionTask(with: request) { [weak self] result, error in
            guard let self else { return }
            if let error {
                let nsErr = error as NSError
                KhanLog.voice.error("recognition error code=\(nsErr.code, privacy: .public) domain=\(nsErr.domain, privacy: .public) desc=\(error.localizedDescription, privacy: .public)")
            }
            Task { @MainActor in
                if let result {
