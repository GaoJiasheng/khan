...
final class IOSVoiceSettings: ObservableObject {
    ...
    @Published var provider: Provider {
        didSet { UserDefaults.standard.set(provider.rawValue, forKey: "doris.ios.voice.provider") }
    }
    @Published var language: VoiceLanguage {
        didSet { UserDefaults.standard.set(language.rawValue, forKey: "doris.ios.voice.language") }
    }
    @Published var copyToClipboard: Bool {
        didSet { UserDefaults.standard.set(copyToClipboard, forKey: "doris.ios.voice.copyToClipboard") }
    }

    private init() {
        let d = UserDefaults.standard
        self.provider = (d.string(forKey: "doris.ios.voice.provider").flatMap(Provider.init(rawValue:))) ?? .chatGPT
        self.language = (d.string(forKey: "doris.ios.voice.language").flatMap(VoiceLanguage.init(rawValue:))) ?? .auto
        self.copyToClipboard = (d.object(forKey: "doris.ios.voice.copyToClipboard") as? Bool) ?? true
    }
}
