...
///
/// Persisted in the **App Group** UserDefaults (`group.com.gavin.khan.shared`)
...
public final class SyncSettings: ObservableObject {
    ...
    private static let cloudKitEnabledKey = "khan.sync.cloudkit.enabled"
    private static let autoSyncEnabledKey = "khan.sync.auto.enabled"
    private static let lastSyncedAtKey    = "khan.sync.lastSyncedAt"
    ...
    private static let store: UserDefaults = {
        UserDefaults(suiteName: KhanIdentifiers.appGroup) ?? .standard
    }()
    ...
    private init() {
        ...
        // KHAN_USE_CLOUDKIT=1 in their shell, treat it as opt-in. We only do
