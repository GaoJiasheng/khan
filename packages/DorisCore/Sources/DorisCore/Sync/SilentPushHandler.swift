...
public final class SilentPushHandler {
    ...
    public init(
        ...
        container: CKContainer = CKContainer(identifier: DorisIdentifiers.cloudKitContainer),
    ...
    public func handleRemoteNotification(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        do {
            ...
            DorisLog.push.error("silent push fetch failed: \(String(describing: error), privacy: .public)")
        }
        return true
    }
    ...
    private func ingest(_ record: CKRecord) async throws {
        guard let originString = record[OutboxRecordKeys.originDeviceID] as? String,
            ...
            DorisLog.push.error("malformed outbox record \(record.recordID.recordName, privacy: .public)")
            return
        }
