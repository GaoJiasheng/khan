...
struct InstallCommand: AsyncParsableCommand {
    ...
    @Option(name: .customLong("to"), help: "Destination path. Default: /usr/local/bin/khan")
    var destination: String = "/usr/local/bin/khan"
    ...
    func run() async throws {
        ...
        if !FileManager.default.fileExists(atPath: parent.path) {
            print("khan: parent directory \(parent.path) does not exist. Create it first or pass --to with another location.")
            throw ExitCode(KhanExit.ioError)
        }
        if FileManager.default.fileExists(atPath: dest) {
            if !force {
                print("khan: \(dest) already exists. Pass --force to replace.")
                throw ExitCode(KhanExit.ioError)
            }
        ...
        do {
            ...
            print("khan: symlinked \(absolute) -> \(dest)")
        } catch {
            FileHandle.standardError.write(Data("khan: install failed: \(error)\n".utf8))
