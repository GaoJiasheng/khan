...
struct NoteCommand: AsyncParsableCommand {
    ...
    struct Add: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            do {
                ...
                dieIO("khan: failed to enqueue note: \(error)")
            }
            if !AppLauncher.isRunning() { _ = AppLauncher.launchIfNeeded() }
            info("khan: queued note \(request.id.uuidString)", quiet: quiet)
        }
    }
    ...
    struct List: AsyncParsableCommand {
        ...
        func run() async throws {
            print("khan: note listing requires the running app to respond via IPC. (v0.1: not yet implemented in CLI; use the app UI.)")
        }
    }
    ...
    struct Show: AsyncParsableCommand {
        ...
        func run() async throws {
            print("khan: show \(id) — not yet implemented in CLI; use the app UI.")
        }
    }
    ...
    struct Edit: AsyncParsableCommand {
        ...
        @Flag(name: .customLong("body-stdin")) var bodyStdin: Bool = false
        func run() async throws {
            print("khan: edit \(id) — not yet implemented in CLI; use the app UI.")
        }
    }
    ...
    struct Remove: AsyncParsableCommand {
        ...
        @Argument var id: String
        func run() async throws {
            print("khan: rm \(id) — not yet implemented in CLI; use the app UI.")
        }
    }
}
