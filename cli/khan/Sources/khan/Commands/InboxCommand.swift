...
struct InboxCommand: AsyncParsableCommand {
    ...
    struct List: AsyncParsableCommand {
        ...
        func run() async throws {
            ...
            do {
                ...
                dieIO("khan: failed to enqueue request: \(error)")
            }
            print("khan: requested inbox list (response streams via app outbox; reader UI not yet implemented in CLI v0.1)")
        }
    }
    ...
    struct Tail: AsyncParsableCommand {
        ...
        func run() async throws {
            print("khan: inbox tail — long-running stream not yet implemented in CLI v0.1; use the app inbox view.")
        }
    }
    ...
    struct Dismiss: AsyncParsableCommand {
        ...
        func run() async throws {
            guard let uuid = UUID(uuidString: id) else { dieUsage("khan: invalid message id") }
            let request = IPCRequest(kind: .inboxDismiss, payload: .inboxDismiss(messageID: uuid))
            try? IPCDirectory.ensureDirectories()
            try IPCWriter.enqueue(request)
            IPCWriter.kick()
            print("khan: dismiss queued")
        }
    }
