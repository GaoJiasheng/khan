...
struct MarkMessageDoneIntent: AppIntent {
    ...
    static var description = IntentDescription("Mark a khan inbox message as actioned.")
    ...
    func perform() async throws -> some IntentResult {
        guard let uuid = UUID(uuidString: messageID) else {
            throw NSError(domain: "khan.intents", code: 1, userInfo: [NSLocalizedDescriptionKey: "invalid uuid"])
        }
